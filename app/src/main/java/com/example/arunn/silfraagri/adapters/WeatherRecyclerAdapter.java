package com.example.arunn.silfraagri.adapters;
...
import com.example.arunn.silfraagri.models.WeatherViewHolder;
    ...
    public void onBindViewHolder(WeatherViewHolder customViewHolder, int i) {
        ...
        if (sp.getString("speedUnit", "m/s").equals("bft")) {
            customViewHolder.itemyWind.setText(context.getString(R.string.wind) + ": " +
                    UnitConvertor.getBeaufortName((int) wind) + " " + Weather_activity.getWindDirectionString(sp, context, weatherItem));
        } else {
            customViewHolder.itemyWind.setText(context.getString(R.string.wind) + ": " + new DecimalFormat("0.0").format(wind) + " " +
                    Weather_activity.localize(sp, context, "speedUnit", "m/s")
                    + " " + Weather_activity.getWindDirectionString(sp, context, weatherItem));
        }
        customViewHolder.itemPressure.setText(context.getString(R.string.pressure) + ": " + new DecimalFormat("0.0").format(pressure) + " " +
                Weather_activity.localize(sp, context, "pressureUnit", "hPa"));
